...
const MarketPulseChart = () => {
  ...
  const normalizeValue = (value) => {
    // Corrected to handle cases where maxValue and minValue are the same
  ...
  const isTrendingUp =
    activeData[0].value < activeData[activeData.length - 1].value;
    ...
    <div className="market-pulse-chart">
      <div className="chart-header">
        ...
        <div className="chart-value">
          ...
          <span
            className={`change-value ${isTrendingUp ? 'positive' : 'negative'}`}
          >
      ...
      <div className="chart-container">
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="chart-svg">
          {/* Area under the line */}
          <motion.path
            key={activeTimeframe + 'area'} // Added key for animation on timeframe change
            d={generateAreaPath()}
          ...
          {/* Chart line */}
          <motion.path
            key={activeTimeframe + 'line'} // Added key for animation on timeframe change
            d={generatePath()}
            fill="none"
                ...
                {activeIndex === index && (
                  <g className="tooltip">
                    <rect
                      x={x}
                      y={y}
                      transform={`translate(${x > 80 ? -60 : 0}, ${y < 20 ? 10 : -30})`} // Dynamically position tooltip
                      width="60"
                      height="20"
                      rx="4"
                    ...
                    <text
                      x={x}
                      y={y}
                      transform={`translate(${x > 80 ? -30 : 30}, ${y < 20 ? 22 : -18})`} // Dynamically position tooltip text
                      textAnchor="middle"
