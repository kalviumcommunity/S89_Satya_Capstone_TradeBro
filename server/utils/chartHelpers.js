...
function generateMockChartData(symbol, options = {}) {
  ...
  // Generate a realistic starting price based on symbol or random
  ...
  // Generate candles going backward in time
  for (let i = 0; i < count; i++) {
    ...
    const extraSpread = Math.random() * volatility * currentPrice;
...
function getDataCountForRange(range, interval = '5min') {
  const rangeMappings = {
    '1D': { '1min': 390, '5min': 78, '15min': 26, '30min': 13, '1h': 7 },
  ...
  return rangeMappings[range]?.[interval] || 100;
}
...
function validateChartParams(symbol, range = '1D', interval = '5min') {
  ...
  } else if (!/^[A-Z]{1,5}(\.[A-Z]{1,3})?$/.test(symbol)) {
